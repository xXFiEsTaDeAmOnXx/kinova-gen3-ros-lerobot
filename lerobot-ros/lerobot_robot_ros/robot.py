...
class ROS2Robot(Robot):
    ...
    def send_action(self, action: dict[str, float]) -> dict[str, float]:
        ...
        if self.config.action_type == ActionType.CARTESIAN_VELOCITY:
            ...
            for joint in self.config.ros2_interface.arm_joint_names:
                ...
                if key in action:
                    ...
                    if not hasattr(self, "_warned_missing_action_keys"):
                        self._warned_missing_action_keys = set()
                    if key not in self._warned_missing_action_keys:
                        logger.warning(
                            f"Action dict missing expected key '{key}'. Using current position to hold place. Keys generated by teleop: {list(action.keys())}"
                        )
                        self._warned_missing_action_keys.add(key)
                    joint_positions.append(joint_state["position"].get(joint, 0.0))
        ...
        if gripper_pos is not None:
            ...
            if not hasattr(self, "_warned_missing_gripper_key"):
                logger.warning("Action dict missing 'gripper.pos'. Not sending gripper command.")
                self._warned_missing_gripper_key = True
